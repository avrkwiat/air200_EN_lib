...
--Module Name: mqtt Protocol Management (SSL)
...
--Parameters:
--???????? idx: number type socket socketx maintained in socketssl, the same as the first parameter passed in when calling socketssl.connect, the program can ignore the non-processing
...
--Parameters:
--???????? idx: Socket socket maintained in socketssl idx, the same as the first argument passed when socketssl.connect is invoked, the program can ignore the non-processing
--???????? data: received data
--Return Value: None
...
--chksvrcrt: boolean type, whether to check the server certificate [optional, default false]
--Return Value: None
